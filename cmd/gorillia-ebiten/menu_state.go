...
func (m *menuState) Draw(g *Game, screen *ebiten.Image) {
	...
	cx := g.Game.Width/2 - 10*charW
	cy := g.Game.Height/2 - 2*charH
	var f1, f2 []string
	if m.stage == 0 {
	...
	ebitenutil.DebugPrintAt(screen, "GORILLAS", (g.Game.Width-8*charW)/2, cy-2*charH)
	if m.stage == 1 {
		line := "V/X - View Intro"
		ebitenutil.DebugPrintAt(screen, line, (g.Game.Width-len(line)*charW)/2, cy+3*charH)
		line = "I - Instructions"
		ebitenutil.DebugPrintAt(screen, line, (g.Game.Width-len(line)*charW)/2, cy+4*charH)
		line = "P/Start - Play Game"
		ebitenutil.DebugPrintAt(screen, line, (g.Game.Width-len(line)*charW)/2, cy+5*charH)
		line = "Q/B - Quit"
		ebitenutil.DebugPrintAt(screen, line, (g.Game.Width-len(line)*charW)/2, cy+6*charH)
	}
}
